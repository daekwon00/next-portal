...
interface MockPost {
  ...
  title: string;
  content: string;
  author: { id: number; username: string; name: string; email: string; role: string };
  viewCount: number;
  files: { id: number; originalName: string; storedName: string; size: number; contentType: string }[];
  createdAt: string;
  updatedAt: string;
}
...
const MOCK_POSTS: MockPost[] = Array.from({ length: 20 }, (_, i) => ({
  ...
  files:
    i % 4 === 0
      ? [
          { id: i + 100, originalName: "첨부파일.pdf", storedName: `stored_${i}.pdf`, size: 1024 * 256, contentType: "application/pdf" },
          { id: i + 101, originalName: "이미지.png", storedName: `stored_${i}.png`, size: 1024 * 128, contentType: "image/png" },
        ]
      : [],
...
export const postHandlers = [
  ...
  http.post("*/api/v1/posts", async ({ request }) => {
    const body = (await request.json()) as { boardId: number; title: string; content: string; fileIds?: number[] };
    const newPost: MockPost = {
      id: nextId++,
      boardId: body.boardId,
      title: body.title,
      content: body.content,
      author: { id: 1, username: "admin", name: "관리자", email: "admin@example.com", role: "ADMIN" },
      viewCount: 0,
      files: [],
  ...
  http.put("*/api/v1/posts/:postId", async ({ params, request }) => {
    const body = (await request.json()) as { title: string; content: string; fileIds?: number[] };
    const post = MOCK_POSTS.find((p) => p.id === Number(params.postId));
    if (!post) {
      return HttpResponse.json({ message: "게시글을 찾을 수 없습니다." }, { status: 404 });
    }
