...
export const userHandlers = [
  ...
  http.get('*/api/v1/users/me/login-history', () => {
    const now = Date.now()
    return HttpResponse.json(
      Array.from({ length: 5 }, (_, i) => ({
        loginIp: i === 2 ? '192.168.1.100' : '127.0.0.1',
        loginSuccess: i !== 2,
        loginFailReason: i === 2 ? '비밀번호 불일치' : null,
        loginDate: new Date(now - i * 3600000 * 8).toISOString(),
      }))
    )
  }),
]
