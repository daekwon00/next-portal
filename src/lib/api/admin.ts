...
export async function updateUser(userId: number, data: UpdateUserRequest) {
  return apiClient.put(`admin/users/${userId}`, { json: data }).json<AdminUser>();
}

export async function toggleUserActive(userId: number) {
  return apiClient.patch(`admin/users/${userId}/toggle-active`).json<AdminUser>();
}
...
export async function toggleBoardActive(boardId: number) {
