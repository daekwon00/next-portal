...
import { adminGetBoards, createBoard, updateBoard, toggleBoardActive } from "@/lib/api/admin";
...
export function useCreateBoard() {
  ...
  return useMutation({
    mutationFn: (data: { name: string; description: string }) => createBoard(data),
...
export function useToggleBoardActive() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (boardId: number) => toggleBoardActive(boardId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin", "boards"] }),
  });
}
